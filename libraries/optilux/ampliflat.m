...
%               one-sided, dual-polarization, power spectral density N0 of  
%               amplified spontaenous emission (ASE)  by N0=h*nu*X.f*G, 
%               with h Planck's constant, nu carrier frequency, G gain.
...
if any(sigma)
    ...
    if length(sigma) > 1 && size(E.field,2) == 2*length(E.lambda)% sep. field & dual polarization
        sigmatmp=repmat(sigma',1,2)'; % duplicate each element
        sigma = sigmatmp(:)';
    end
