function varargout=datapattern(Nsymb,ptype,varargin)

%DATAPATTERN sequence pattern
%   PAT=DATAPATTERN(NSYMB,PTYPE,NSEED) returns in PAT a sequence of NSYMB
...
%               - For NSEED >= NSYMB/4 the DBS is not unique, hence 
%               DATAPATTERN generates a circular random delayed version of 
%               a DBS with NSEED < NSYMB/4.
...
%
%               PAT=DATAPATTERN(NSYMB,'rand') does not set the random seed,
%               which can thus be set before calling DATAPATTERN with RNG 
%               or RAND with the field 'state'.
%
%               PAT=DATAPATTERN(NSYMB,'rand',OPTIONS) does not set the 
%               random seed and uses optional arguments in OPTIONS (see 
%               later).
%
%               PAT=DATAPATTERN(NSYMB,'rand',NSEED) sets the state of the
%               random generator to NSEED. Please note that in such a case
%               the original random seed is restored after the call to
%               DATAPATTERN.
%
%               PAT=DATAPATTERN(NSYMB,'rand',NSEED,OPTIONS) is the most
...
%
%               PAT = DATAPATTERN(NSYMB,<sequence of numbers>,OPTIONS) uses
...
%           E.g. PAT=DATAPATTERN(NSYMB'debruijn',0,OPTIONS) with
...
%
%           E.g. PAT=DATAPATTERN(NSYMB,'rand',OPTIONS) with
...
%
%
%   [PAT,BMAT] = DATAPATTERN(NSYMB,PTYPE,NSEED,OPTIONS) returns in BMAT the
...
if strcmp(ptype,'debruijn') %%%%% DE BRUIJN SEQUENCE
    ...
    if nargin < 3
        ...
        nseed = varargin{1};
        if isstruct(nseed), error('The seed must be an integer. Did you set the seed?'); end
...
elseif isnumeric(ptype)     %%%%% VECTOR OF SYMBOLS
    if length(ptype) > Nsymb
        warning('optilux:datapattern','Pattern too long: truncated.');
    end
...
elseif (min(double(ptype-48)) >= 0 && max(double(ptype-48)) <= 9)   %%%%% STRING
    ...
    if nargin >= 4
        error('Too many input arguments.');
    elseif nargin == 3
        [pat,patmat] = datapattern(Nsymb,ptype2,varargin{end});
    else
        [pat,patmat] = datapattern(Nsymb,ptype2);
    end
    varargout{2} = patmat;
