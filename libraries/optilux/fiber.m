...
if x.isuni % unique field
    ...
    if x.isy % Add DGD on polarizations
        betat(:,ipm+1) = betat(:,ipm) - dgd1*omega;
    end
else % separate-field
...
if x.isuni %%% UNIQUE FIELD
    ...
    if ~x.ismanakov && size(u,2) > 1 % CNLSE, only in dual-polarization
        ...
        for nch=1:Npol:nfc % forward
            f11 = zeros(Nfft,1); if x.isy,f12=f11; f22=f11;end
...
%                     f22 = f22+gamleff(inch)*Pow(:,kch);
                    if x.isy
                        ...
                        f22 = f22+gamleff(inch)*(Pow(:,kch)+Pow(:,kch+1));
                    end
                else
...
%   [DZB,NINDEX]=CHECKSTEP(ZPROP,DZ,LCORR) split the step DZ [m] into a
%   sequence of sub-steps, each of maximum length equal to LCORR.
%
%   LCORR is the waveplate (trunk) length [m], i.e., the length over which
%   the birefringence is supposed to be constant. ZPROP [m] is the 
%   coordinate of step-end, DZ [m] is the step length.
%
%   On output, DZB is a vector containing all the sub-steps within DZ. It
%   is sum(DZB) = DZ.
%
%   For instance, with LCORR = 1 [m] and a step starting at coordinate
%   8.5 [m] with DZ=1.8 [m]. The step DZ crosses 3 waveplates yielding
%   DZB = [0.5, 1, 0.3] and NINDEX = [9 10 11].
%
%   ---------------------------------------------------------------
%            |      *     |           |    *      |
%  ZPROP:    8     8.5    9           10  10.3    11    [m]
%  TRUNK:    9            10          11          12
%  INDEX:     (NINI) 9    9 10       10 11  11 (NEND)
%

%   All lengths are in [m].

TOL0 = 1e-9;    % [m] tolerance to claim a step of zero length
...
% Note: for instance, floor(10)+1=11, but ceil(10)=10
...
dzb = dzsplit(abs(dzsplit) > TOL0); % remove zero-length steps
nindex = nmid(abs(dzsplit) > TOL0); % remove zero-length steps
