...
options = [];
errmsg=checkfields(E,{'lambda','field'});
if ~isempty(errmsg), error('optilux:multiplexer',errmsg);end
...
if nargin >= 2
    ...
    if isfield(o1,'field') % Concatenate the structs E and o1
        errmsg=checkfields(o1,{'lambda','field'});
        if ~isempty(errmsg), error('optilux:multiplexer',errmsg);end
...
tmax = 1/minfreq; % [ns]
tim = (0:tmax/GSTATE.NSAMP:tmax-tmax/GSTATE.NSAMP)'; % time axis [ns]

Eout = zeros(Nfft,Npol);
for kch=1:length(lambda)    % multiplexing
    expimod = fastexp(-2*pi*deltafn(kch).*tim); % kch-th carrier
    if Npol == 1
        Eout = Eout + fft(E.field(:,kch) .* expimod);
    else
        Eout(:,1) = Eout(:,1) + fft(E.field(:,2*kch-1) .* expimod);
        Eout(:,2) = Eout(:,2) + fft(E.field(:,2*kch) .* expimod);
    end
end
